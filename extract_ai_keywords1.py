...
OUT_DIR = r"D:\\out" # Folder containing per-year outputs like: 2010_skills_counts_co.parquet
TMP_DIR = r"D:\\duckdb_tmp" # DuckDB spill/temp directory (ensure plenty of space; put on SSD/NvMe) 
MEM_LIMIT = "24GB" # Memory cap for DuckDB
SAVE_AS = "parquet" # Final output format: "parquet" or "csv" 
MIN_JOBS = 20 # optional: filter out very rare skills in the global table 
...
parquet_files = sorted(glob.glob(f"{out_dir}/[0-9][0-9][0-9][0-9]_skills_counts_co.parquet"))
csv_files     = sorted(glob.glob(f"{out_dir}/[0-9][0-9][0-9][0-9]_skills_counts_co.csv"))

if parquet_files:
    # build a DuckDB list literal of file paths: ['path1', 'path2',...]
    # read_parquet([...], union_by_name=true) loads many files as one table (column-aligned by name)
    src_list  = "[" + ",".join(f"'{norm(p)}'" for p in parquet_files) + "]"
    read_src  = f"read_parquet({src_list}, union_by_name=true)"
    src_kind  = "parquet"
elif csv_files:
    # read_csv_auto([...], union_by_name=true) similarly unions many CSVs; auto-infers column types 
    src_list  = "[" + ",".join(f"'{norm(p)}'" for p in csv_files) + "]"
    read_src  = f"read_csv_auto({src_list}, union_by_name=true)"
    src_kind  = "csv"
else:
    raise FileNotFoundError("No yearly files like YYYY_skills_counts_co.(parquet|csv) found in OUT_DIR.")

print(src_list)
  
copy_opts = "(FORMAT 'parquet', COMPRESSION 'ZSTD')" if SAVE_AS == "parquet" else "(HEADER, DELIMITER ',')"
out_all   = f"{out_dir}/all_years_skills_counts_co.{'parquet' if SAVE_AS=='parquet' else 'csv'}" # global table
out_top   = f"{out_dir}/top100_ai_skills_all_years.{'parquet' if SAVE_AS=='parquet' else 'csv'}" # top-100 view 

# optional WHERE clause: keep only skills with sufficient global support (total_cnt >= MIN_JOBS)
